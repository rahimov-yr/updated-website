...
# Check if program_sections exists
cursor.execute('SELECT setting_key FROM site_settings WHERE setting_key = ?', ('program_sections',))
exists = cursor.fetchone()

if exists:
    # Update existing record
    cursor.execute('UPDATE site_settings SET setting_value = ?, updated_at = datetime("now") WHERE setting_key = ?',
                   (json.dumps(program_sections), 'program_sections'))
    print("Updated existing program_sections")
else:
    # Insert new record
    cursor.execute('INSERT INTO site_settings (setting_key, setting_value, updated_at) VALUES (?, ?, datetime("now"))',
                   ('program_sections', json.dumps(program_sections)))
    print("Inserted new program_sections")

conn.commit()
conn.close()
print("Program sections populated successfully!")
