...
// Program sections
const SECTIONS = {
  structure: { key: 'structure', name: 'Структура программы' },
  plenary: { key: 'plenary', name: 'Пленарное заседание' },
  events: { key: 'events', name: 'Мероприятия в рамках конференции' },
  forums: { key: 'forums', name: 'Форумы' }
}
...
export default function SimpleProgramManager({ embedded = false }) {
  const { token } = useAuth()
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (settingsMap.program_sections) {
        const parsed = JSON.parse(settingsMap.program_sections)

        // Ensure each section has a blocks array
        const sectionsWithBlocks = {
          structure: { blocks: [], ...(parsed.structure || {}) },
          plenary: { blocks: [], ...(parsed.plenary || {}) },
          events: { blocks: [], ...(parsed.events || {}) },
          forums: { blocks: [], ...(parsed.forums || {}) }
        }
  ...
  const handleSave = async () => {
    ...
    try {
      await saveSettingsApi('program_sections', JSON.stringify(sections))
  ...
  const addBlock = (type) => {
    ...
    setSections(prev => ({
      ...
      [activeSection]: {
        ...prev[activeSection],
        blocks: [...(prev[activeSection]?.blocks || []), newBlock]
      }
    }))
  ...
  const updateBlock = (blockId, updates) => {
    setSections(prev => ({
      ...
      [activeSection]: {
        ...prev[activeSection],
        blocks: (prev[activeSection]?.blocks || []).map(block =>
  ...
  const deleteBlock = (blockId) => {
    if (confirm('Удалить этот блок?')) {
      setSections(prev => ({
        ...
        [activeSection]: {
          ...prev[activeSection],
          blocks: (prev[activeSection]?.blocks || []).filter(block => block.id !== blockId)
        }
      }))
  ...
  return (
    <div className="elementor-builder">
      {/* Section Tabs at Top */}
      <div className="elementor-topbar-combined">
        <div className="elementor-page-tabs-only">
          <div className="page-tabs-container">
            {Object.values(SECTIONS).map(section => (
              <button
                key={section.key}
                className={`page-tab ${activeSection === section.key ? 'active' : ''}`}
      ...
      {/* Workspace with Sidebar and Canvas */}
